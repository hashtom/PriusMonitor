...
public class OBD2Adapter {
	...
	public OBD2Adapter(BluetoothDevice bt) {
	...
	public OBD2Diagnosis getOBD2Diagnosis(DiagnosisOptions options){
		...
		for (PidParameter param : options) {
			if(param.isActivated){
				switch (param.obd2id) {
				case BatteryVoltage:
					outElmATValue = elmConnect.sendElm327ATCommand(param.ATCommand);
					diagnosis.batteryVoltage = 12.5; //文字列から数値変換へ
					break;
				case Watertemp:
					outOdbValue = elmConnect.sendOdb2Command(param.mode,param.pid);
